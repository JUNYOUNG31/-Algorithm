...
# h의 최댓값이 이 과학자의 H-Index입니다.어떤 과학자가 발표한 논문의 인용 횟수를 담은 배열 citations가
...
def solution(citations):
    answer = 0
    citations.sort()                          # 정렬
    length = len(citations)                   # 길이
    for i in range(length):                   # 반복
        # if citations[i] <= length - i:
        #     answer = citations[i] ????
        if citations[i] >= length - i:        # H 찾기
            answer = max(answer, length - i)  # max H 갱신
    return answer
