...
word = input()                   # 입력
pad = ['0', '0', '1', 'ABC', 'DEF', 'GHI', 'JKL', 'MNO', 'PQRS', 'TUV', 'WXYZ'] # 다이얼 패드
ans = 0                          # 카운트할 시간
for j in range(len(word)):       # 단어의 길이 만큼 반복해서
    for i in pad:                # 패드안에서
        if word[j] in i:         # 할머니가 외운 단어가 패드안에 있다면
            ans += pad.index(i)  # 그 패드의 index 를 답에 더한다
print(ans)
